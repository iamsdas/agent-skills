...
3. Refine expectations without implementation details:
   - **HARD GATE: do NOT invoke AskUserQuestion until the "Product Overview" and "What I Found" sections (Response Template below) have been output as visible text in the conversation.** This applies equally when this skill runs as a sub-skill of another workflow (e.g. `setup-task`) — having read the ticket is not a substitute for showing the user the summary.
...
- The first AskUserQuestion call must come AFTER a message containing the "Product Overview" and "What I Found" summary — even when invoked from another skill like `setup-task`.
