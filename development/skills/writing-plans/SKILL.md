...
**Seed the KB for execution (when an explorer agent runs).** Pick the run's KB source label now — `orient:<feature-name>` (same kebab-case slug used for the plan files). In each `code-explorer`'s dispatch prompt, make KB seeding an **integral, up-front part of the task** — not an aside bolted onto "explore and report." State it as: *"Your dispatch supplies KB source label `orient:<feature-name>`; per your Knowledge Base Seeding instructions, `ctx_index` the code you explore under it."* Naming the label as a supplied parameter and pointing at the explorer's own charter is what keeps it from reading the instruction as prompt injection and falling back to Glob/Grep/Read. (The explorer carries `ctx_index`/`ctx_search` and loads the deferred schema once via `ToolSearch`.) The default executor, `subagent-driven-development`, reuses this label so it does **not** re-explore the codebase from scratch — discovery is paid once, here. Record the label in the plan header so the handoff can pass it on. If you explored by reading files yourself (no agent dispatched), skip seeding — there's no label to pass, and execution will do its own orientation.
